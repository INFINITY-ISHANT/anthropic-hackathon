...
Geography note: states/districts/constituencies are read from JSON for the
demo's curated subset. To bulk-load all 543 Lok Sabha seats, drop a CSV at
data/seed/constituencies.csv with columns (name, type, number, state_code,
district_name) and it will be merged in alongside the JSON. Same for
states.csv and districts.csv.
"""
...
def _load_csv(name: str) -> list[dict]:
    """Optional CSV loader. Missing file is fine — returns []."""
    path = SEED_DIR / name
    if not path.exists():
        return []
        ...
        for r in reader:
            ...
            if r.get("number"):
                try:
                    ...
                    r["number"] = None
            rows.append({k: (v.strip() if isinstance(v, str) else v) for k, v in r.items() if v not in (None, "")})
        logger.info("Loaded %d rows from %s", len(rows), name)
        return rows
...
def seed_all() -> None:
    ...
    db = SessionLocal()
    try:
        # 1. States — keyed by `code`. JSON + optional CSV merged.
        for row in _load("states.json") + _load_csv("states.csv"):
            _upsert_by(db, State, "code", row)

        # 2. Districts — keyed by (name, state_code) — resolve state_code → state_id
        for row in _load("districts.json") + _load_csv("districts.csv"):
        ...
        for row in _load("constituencies.json") + _load_csv("constituencies.csv"):
            state = db.execute(select(State).where(State.code == row.pop("state_code"))).scalar_one()
            district = None
