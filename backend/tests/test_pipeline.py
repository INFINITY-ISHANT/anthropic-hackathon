...
def test_pipeline_ingests_mock_documents():
    ...
    # Keep tests offline: deactivate every non-mock source. The seed has them
    # active by default for the demo, but tests shouldn't touch the network.
    with SessionLocal() as db:
        for s in db.query(Source).all():
            if s.adapter_key != "mock":
                s.active = False
        db.commit()
