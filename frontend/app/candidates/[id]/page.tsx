...
export default async function CandidatePage({ params }: PageProps) {
  ...
  let brief;
  try {
    [c, brief] = await Promise.all([
      api.candidate(id),
      api.candidateBrief(id).catch(() => null),
    ]);
  } catch {
    notFound();
  }
  ...
  return (
    <div>
      ...
      {brief && (
        <section className="my-8 p-6 border border-paper-line bg-paper-deep rounded-sm">
          <p className="label-caps mb-3">Voter Summary · AI-generated from public records</p>
          {brief.voter_summary && (
            <p className="text-ink-soft leading-relaxed text-lg max-w-column">
              {brief.voter_summary}
            </p>
          )}
          {brief.notable_facts.length > 0 && (
            <ul className="mt-4 space-y-1">
              {brief.notable_facts.map((f, i) => (
          ...
          <div className="mt-4 flex items-center gap-4 flex-wrap">
            ...
            <span className={`text-[11px] uppercase tracking-wideish px-2 py-0.5 rounded-sm font-medium ${
              brief.data_completeness === "full" ? "bg-green-soft text-green" :
              brief.data_completeness === "partial" ? "bg-saffron-soft text-saffron" :
              "bg-paper-deep text-ink-muted border border-paper-line"
            }`}>
              {brief.data_completeness} data
            </span>
            {brief.promise_completion_rate !== null && (
              <span className="text-[11px] uppercase tracking-wideish text-ink-muted">
                {brief.promise_completion_rate}% promises kept
              </span>
            )}
          </div>
          <p className="mt-3 text-[11px] text-ink-muted">
            Generated from ECI affidavits, manifesto records, and parliamentary data.
            Verify with the original sources linked below.
          </p>
        </section>
      )}
