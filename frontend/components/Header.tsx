...
const NAV = [
  ...
  { href: "/claim-check", label: "Claim Check" },
  { href: "/budget-calculator", label: "Budget Impact" },
  { href: "/rights", label: "Rights Navigator" },
  ...
  return (
    <header className="border-b border-paper-line bg-paper">
      ...
      <div className="max-w-6xl mx-auto px-6 pt-8 pb-4 flex items-end justify-between">
        ...
        <div className="hidden md:flex flex-col items-end gap-3 max-w-xs">
          <LanguageSwitcher />
      ...
      {/* Navigation row */}
      <nav className="max-w-6xl mx-auto px-6 py-3 border-t border-b border-ink/90 flex flex-wrap gap-x-6 gap-y-2 text-sm overflow-x-auto">
        {NAV.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="text-ink-soft hover:text-ashoka transition-colors font-medium whitespace-nowrap"
          >
            {item.label}
          </Link>
        ))}
